...
stationsmap <- leaflet() %>% 
  addTiles() %>% 
  setView(lng = -6.5669, lat = 36.5533, zoom = 2.45) %>% 
  ...
  addCircleMarkers(lng = -3.9375, lat = 48.7778, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_002") %>% 
  addCircleMarkers(lng = 7.9, lat = 54.18194, color = "#FA776B", radius = 3, opacity = 1, label = "OSD_003") %>% 
  addCircleMarkers(lng = 3.14, lat = 42.49, color = "#F95C74", radius = 3, opacity = 1, label = "OSD_014") %>% 
  addCircleMarkers(lng = 5.1746, lat = 43.22639, color = "#F95C74", radius = 3, opacity = 1, label = "OSD_022") %>% 
  addCircleMarkers(lng = 23.2538, lat = 59.8822, color = "#FDBBB5", radius = 3, opacity = 1, label = "OSD_030") %>% 
  addCircleMarkers(lng = -80.09315, lat = 26.10293, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_037") %>% 
  addCircleMarkers(lng = -79.89954, lat = 32.7524, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_039") %>% 
  addCircleMarkers(lng = -117.25725, lat = 32.86698, color = "#F5005A", radius = 3, opacity = 1, label = "OSD_043") %>% 
  addCircleMarkers(lng = 13.33189, lat = 45.325568, color = "#F95C74", radius = 3, opacity = 1, label = "OSD_049") %>% 
  addCircleMarkers(lng = -69.6409, lat = 43.8444, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_054") %>% 
  addCircleMarkers(lng = -69.5781, lat = 43.8604, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_055") %>% 
  addCircleMarkers(lng = -79.16763,	lat = 33.32306, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_060") %>% 
  addCircleMarkers(lng = 10, lat = 54.8333, color = "#FDBBB5", radius = 3, opacity = 1, label = "OSD_072") %>% 
  addCircleMarkers(lng = 12.935,	lat = 43.9475, color = "#F95C74", radius = 3, opacity = 1, label = "OSD_076") %>% 
  addCircleMarkers(lng = 13.0731, lat = 43.8514, color = "#F95C74", radius = 3, opacity = 1, label = "OSD_077") %>% 
  addCircleMarkers(lng = -20.3043, lat = 74.31, color = "#7C0068", radius = 3, opacity = 1, label = "OSD_080") %>% 
  addCircleMarkers(lng = 13.71003, lat = 45.70092, color = "#F95C74", radius = 3, opacity = 1, label = "OSD_099") %>% 
  addCircleMarkers(lng = 32.954, lat = 32.822, color = "#F95C74", radius = 3, opacity = 1, label = "OSD_123") %>% 
  addCircleMarkers(lng = 135.12083, lat = 34.32444, color = "#F5005A", radius = 3, opacity = 1, label = "OSD_124") %>% 
  addCircleMarkers(lng = 34.843, lat = 32.0694, color = "#F95C74", radius = 3, opacity = 1, label = "OSD_132") %>% 
  addCircleMarkers(lng = 5.11504, lat = 60.16121, color = "#FA776B", radius = 3, opacity = 1, label = "OSD_141") %>% 
  addCircleMarkers(lng = -81.01667, lat = 31.98282, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_143") %>% 
  addCircleMarkers(lng = -2.829667, lat = 78.453333, color = "#7C0068", radius = 3, opacity = 1, label = "OSD_146") %>% 
  addCircleMarkers(lng = -54.266, lat = -34.616, color = "#AA006A", radius = 3, opacity = 1, label = "OSD_149") %>% 
  addCircleMarkers(lng = -54.2752, lat = -34.666, color = "#AA006A", radius = 3, opacity = 1, label = "OSD_150") %>% 
  addCircleMarkers(lng = -63.6403, lat = 44.6936, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_152") %>% 
  addCircleMarkers(lng = -4.552, lat = 48.359, color = "#FB9C70", radius = 3, opacity = 1, label = "OSD_159") %>% 
  addMeasure(position = "bottomright", primaryLengthUnit = "kilometers") %>% 
  addScaleBar(position = "bottomleft")
saveWidget(stationsmap, file = "X:/Documents/OSD/OSD/complete-stations-map.html")
```

## Dataframes
```{r, echo=TRUE, message=TRUE, warning=TRUE}
#DF with identified ASV by DADA2
OSD <- read.csv("X:/Documents/OSD/OSD/seqtab_nonchim.csv", header = T, sep = ";", dec = '.', skip = 0, fill = TRUE)
OSD <- mutate(OSD, 
               Sample = c("OSD_002", "OSD_003", "OSD_014", "OSD_022", "OSD_30", "OSD_037", "OSD_039", "OSD_043", "OSD_049", "OSD_054", "OSD_055", "OSD_060", "OSD_072", "OSD_076", "OSD_077", "OSD_080", "OSD_099", "OSD_123", "OSD_124", "OSD_132", "OSD_141", "OSD_143", "OSD_146", "OSD_149", "OSD_150", "OSD_152", "OSD_159"), 
               .after = "X")
SN <- OSD[,2]
OSD <- OSD[,-c(1, 2)]
OSD <- apply(OSD, 2, function(x) as.numeric(as.integer(x))) %>% as.data.frame()
rownames(OSD) <- SN
write.csv2(OSD, file = "X:/Documents/OSD/OSD/osd_asv.csv") #Saving some DF necessary for downstream analysis

#ASV assigned taxa from PR2 4.14 
taxa <- read.csv("X:/Documents/OSD/OSD/taxa_osd.csv", header = T, sep = ";", dec = '.', skip = 0) %>% rename(Seq = X) %>% add_column(OTU = c(paste0("ASV_", 1:2834)), .after = "Seq")
rownames(taxa) <- taxa[,1]
write.csv2(taxa, file = "X:/Documents/OSD/OSD/osd_taxa.csv")

print(all(colnames(OSD)%in%rownames(taxa))) #If TRUE you can continue
```
...
write.csv2(na.taxa, file = "X:/Documents/OSD/OSD/na_taxa.csv")
na.asv <- OSD %>% select(-all_of(NAseq))
write.csv2(na.asv, file = "X:/Documents/OSD/OSD/na_asv.csv")
```
...
```{r, echo=TRUE, message=TRUE, warning=TRUE}
rare.full <- sort(rowSums(OSD))
rare.full
plot(rare.full)

rrfy<- rarefy(OSD, rare.full[1]) #rarefaction uses the smallest number of observations per sample to extrapolate the expected number if all other samples only had that number of observations
rrfy #gives an "expected"rarefied" number of species (not obs) if only 41641 individuals were present per sample

kols <- colorRampPalette(c("#FB9C70", "#FA776B", "#F95C74", "#F95C74", "#FDBBB5", "#FB9C70", "#FB9C70", "#F5005A", "#F95C74", "#FB9C70", "#FB9C70", "#FB9C70", "#FDBBB5", "#F95C74", "#F95C74", "#7C0068", "#F95C74", "#F95C74", "#F5005A", "#F95C74", "#FA776B", "#FB9C70", "#7C0068", "#AA006A", "#AA006A", "#FB9C70", "#FB9C70"))(27)

tiff("Rarecurve_OSD.tiff", width = 12, height = 6, units = 'in', res = 600)
par(mar=c(5.1,4.1,4.1,2.1), oma=c(0,0,0,0))
rarecurve(x = OSD, col = kols, lwd = 2, las = 1, label = TRUE, step = 10, cex = 0.5)
dev.off()
...
tiff("RC_NA_TARA.tiff", width = 10, height = 7, units = "in", res = 600)
par(mar=c(5.1,4.1,4.1,2.1), oma=c(0,0,0,0))
rarecurve(x = na.asv, col = kols, lwd = 2, las = 1, label = TRUE, step = 10, cex = 0.5)
dev.off()

naOSD <- as.data.frame(rrarefy(na.asv, rare.na[1]))
write.csv2(naOSD, file = "X:/Documents/OSD/OSD/rarefacted_na_osd.csv")
```

```{r, echo=TRUE, message=TRUE, warning=TRUE}
eOSD <- as.data.frame(t(na.asv))
eOSD <- eOSD  %>% 
  mutate(Seq = all_of(rownames(eOSD)), .before = "OSD_002") 
eOSD <- eOSD %>%
  mutate(NAO = rowSums(eOSD[c(2,7,8,11,12,13,23,27,28)]), 
         MS = rowSums(eOSD[c(4,5,10,15,16,18,19)]), 
         NS = rowSums(eOSD[c(3,22)]), 
         BS = rowSums(eOSD[c(6,14)]), 
         NPO = rowSums(eOSD[c(9,20)]), 
         AO = rowSums(eOSD[c(17,24)]), 
         SAO = rowSums(eOSD[c(25,26)]), 
         Total = rowSums(eOSD[2:28])
         )
ATs <- inner_join(eOSD, taxa, by = "Seq") %>% relocate(OTU, .after = Seq)
eSN <- ATs[, 1]
rownames(ATs) <- all_of(eSN)
write.csv2(ATs, file = "X:/Documents/OSD/OSD/at_osd.csv")
...
write.csv2(Phylum, file = "X:/Documents/OSD/OSD/phylum_osd.csv")

ph.osd <- read.csv("X:/Documents/OSD/OSD/phylum_osd.csv", header = TRUE, sep = ";", dec = ".", skip = 0, row.names = 1) %>% rename(Cryp = Cryptophyta, Crypx = Cryptophyta.nucl)
ph.osd <- mutate(ph.osd, 
                  Cryptophyta = rowSums(ph.osd[c(6, 18)]), 
                  .after = Ciliophora
                  )
ph.osd <- ph.osd[, -c(7, 19)]
write.csv2(ph.osd, file = "X:/Documents/OSD/OSD/ephylum_osd.csv")
phyla <- as.data.frame(t(ph.osd))
write.csv2(phyla, file = "X:/Documents/OSD/OSD/phyla_osd.csv")

phyla <- read.csv("X:/Documents/OSD/OSD/phyla_osd.csv", header = TRUE, sep = ";", dec = ".", skip = 0)
phyla <- phyla %>% 
  mutate(NAO = rowSums(phyla[c(2,7,8,11,12,13,23,27,28)]), 
         MS = rowSums(phyla[c(4,5,10,15,16,18,19)]), 
         NS = rowSums(phyla[c(3,22)]), 
         BS = rowSums(phyla[c(6,14)]), 
         NPO = rowSums(phyla[c(9,20)]), 
         AO = rowSums(phyla[c(17,24)]), 
         SAO = rowSums(phyla[c(25,26)]), 
         Total = rowSums(phyla[2:28])
         )
phyla <- arrange(phyla, desc(Total))
phyla <- phyla %>% 
  mutate(Color = c("#002C3D", "#00564E", "#007F5F", "#359548", "#6AAB30", "#BACC0C", "#F1F500", "#ACDD3C", "#83E377", "#16DB93", "#FB9C70", "#FB9262", "#FA8072", "#FA776B", "#F95C74", "#F8417D", "#AA006A", "#7C0068", "#4E0066", "#410151", "#C5DBEA", "#8BB7D4", "#5193BE", "#176EA8", "#00609F", "#023E8A", "#000078", "#161664", "#17315F", "#000000"))
write.csv2(phyla, file = "X:/Documents/OSD/OSD/ephyla_osd.csv")

tiff("Contribution_Distribution_Phylum_OSD.tiff", width = 17, height = 15, units = 'in', res = 600)
treemap(phyla, 
        index = "X", 
        vSize = "Total", 
        type = "color",
        ...
        fontsize.labels = 20,
        fontsize.title = 30,
        title = "Coastal OSD 18S rRNA V4",
        title.legend = NA,
        border.col = NA
)
dev.off()

tiff("Contribution_Distribution_Phylum_OSD_Legend.tiff", width = 10, height = 7, units = 'in', res = 600)
plot.new()
...
relative <- as.data.frame(t(ph.osd))
...
write.csv2(relative, file = "X:/Documents/OSD/OSD/relative_abundance_osd.csv")
#rltvTSDiv <- rltvTSDiv[, -11]

tiff("Relative_Abundance_Phylum_OSD.tiff", width = 10, height = 8, units = 'in', res = 600)
par(mar = c(5.1,4.1,4.1,2.1), oma = c(0,0,0,0))
...
tiff("Relative_Abundance_Phylum_OSD_Legend.tiff", width = 10, height = 7, units = 'in', res = 600)
plot.new()
par(mar = c(0,0,0,0), oma = c(0,0,0,0))
...
write.csv2(edATs, file = "X:/Documents/OSD/OSD/edited_ats_osd.csv")
print(all(all_of(edATs$Seq)%in%colnames(na.asv)))

APATs <- edATs[, -1]
APATs <- decostand(APATs, method = "pa")
APATs <- add_column(APATs, Seq = all_of(edATs$Seq), .before = "OSD_002")
write.csv2(APATs, file = "X:/Documents/OSD/OSD/auspres_osd.csv")
```

##UpSetR
```{r, echo=TRUE, message=TRUE, warning=TRUE}
aus.pres <- read.csv("X:/Documents/OSD/OSD/auspres_osd.csv", header = TRUE, sep = ";", dec = ".", skip = 0)
tiff("UpSetR_EX.tiff", width = 17, height = 11, units = 'in', res = 600)
upset(aus.pres, 
      nsets = 7, 
      nintersects = NA, 
      sets = c("NAO", "NS", "MS", "BS", "NPO", "AO", "SAO"), 
      keep.order = TRUE,
      query.legend = "top",
      queries = list(list(query = intersects, params = "NAO", color = "#00609F", active = TRUE, query.name = "Unique North Atlantic Ocean ASV"),
                     list(query = intersects, params = "NS", color = "#FFB800", active = TRUE, query.name = "Unique North Sea ASV"),
                     list(query = intersects, params = "MS", color = "#F5005A", active = TRUE, query.name = "Unique Mediterranean Sea ASV"),
                     list(query = intersects, params = "BS", color = "#000078", active = TRUE, query.name = "Unique Baltic Sea ASV"),
                     list(query = intersects, params = "NPO", color = "#410151", active = TRUE, query.name = "Unique North Pacific Ocean ASV"), 
                     list(query = intersects, params = "AO", color = "#00564E", active = TRUE, query.name = "Unique Artic Ocean ASV"), 
                     list(query = intersects, params = "SAO", color = "#16DB93", active = TRUE, query.name = "Unique South Atlantic Ocean ASV"), 
                     list(query = intersects, params = list("NAO", "NS", "MS", "BS", "NPO", "AO", "SAO"), color = "#DC2F02", active = TRUE, query.name = "Shared Sites ASV")), 
      point.size = 3, 
      line.size = 1, 
      text.scale = 1.6,
