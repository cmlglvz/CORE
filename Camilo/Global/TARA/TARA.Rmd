...
CORE <- read.csv("/Documents/GitHub/CORE/Camilo/Global/TARA/seqtab_nonchim.csv", 
                 header = TRUE, 
                 sep = ";", 
                 skip = 0, 
                 row.names = 1)
...
write.csv2(CORE, "/Documents/GitHub/CORE/Camilo/Global/TARA/TARA.csv")

TAXA <- read.csv("/Documents/GitHub/CORE/Camilo/Global/TARA/taxa.csv", 
                 header = TRUE, 
                 sep = ";", 
                 skip = 0, 
                 row.names = 1)
...
write.csv2(sTAXA, "/Documents/GitHub/CORE/Camilo/Global/TARA/selected_TAXA.csv")
sTARA <- select(CORE, -all_of(NASeq))
print(all(colnames(sTARA)%in%rownames(sTAXA)))
write.csv2(sTARA, "/Documents/GitHub/CORE/Camilo/Global/TARA/selected_TARA.csv")
```


#Rarefacción
```{r Rarefaction, echo=TRUE, message=TRUE, warning=TRUE}
rare.all <- sort(rowSums(sTARA)) #To identify the sample with the smallest number of observations, which will be used as the number of observations to rarefy samples

rrfy <- rarefy(sTARA, rare.all[1]) #rarefaction use the smallest number of observations per sample to extrapolate the expected number if all other samples only had that number of observations
...
png("/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/Figura_8_Rarecurve_sTARA.png", width = 12, height = 6, units = "in", res = 600)
par(mar = c(5.1, 4.1, 4.1, 2.1), oma = c(0, 0, 0, 0))
rarecurve(x = sTARA, col = "blue", lwd = 2, las = 1, step = 10, cex = 0.5, label = FALSE)
dev.off()

rXP <- as.data.frame(rrarefy(sTARA, rare.all[1]))
write.csv2(rXP, file = "Rarefacted_TARA_ASV_Abundance_V2.csv")

png("/Documents/GitHub/CORE/Camilo/EstExp/Figuras/Figura_9_Rarecurve_rXP.png", width = 12, height = 6, units = "in", res = 600)
par(mar = c(5.1, 4.1, 4.1, 2.1), oma = c(0, 0, 0, 0))
rarecurve(x = rXP, col = "darkgreen", lwd = 2, las = 1, step = 10, cex = 0.5, label = FALSE, ylab = ASV)
dev.off()
...
PreU <- mutate(PreU, Seq = all_of(rownames(PreU)), .before = "TARA109")
PreU <- mutate(PreU, 
               CEPO = rowSums(PreU[2:3]), 
               SWAO = rowSums(PreU[4:6]), 
               MS = rowSums(PreU[7:12]), 
               RS = rowSums(PreU[13:14]), 
               SWIO = rowSums(PreU[15:19]), 
               NEAS = rowSums(PreU[20:22]), 
               Total = rowSums(PreU[2:22])
               )
pre.comp <- inner_join(PreU, TAXA, by = "Seq") %>% relocate(ASV, .after = Seq)
rownames(pre.comp) <- all_of(pre.comp[,1])
write.csv2(pre.comp, "original_unfiltered_composite.csv")
tara <- pre.comp %>% dplyr::filter(TARA007 > 0)
...
```{r}
Phylum <- Tax.sum(rXP, sTAXA, 5) %>% as.data.frame()
Phylum <- Phylum[,-11]
colnames(Phylum)[c(7,10,13,30,31,33,36,41)] <- c("A", "B", "C", "D", "E", "F", "G", "H")
Phylum <- mutate(Phylum, 
                 Cryptophyta = rowSums(Phylum[c(10,36)]), 
                 Streptophyta = rowSums(Phylum[c(33,41)]), 
                 Haptophyta = rowSums(Phylum[c(13,31)]), 
                 Chlorophyta = rowSums(Phylum[c(7,30)])
                 )
Phylum <- Phylum[,-c(7,10,13,30,31,33,36,41)]
Phylum <- as.data.frame(t(Phylum))
Phylum <- mutate(Phylum, Total = rowSums(Phylum[c(1:21)]), .after = "TARA042")
Phylum <- Phylum[order(Phylum$Total, decreasing = TRUE),]
write.csv2(Phylum, "TARA_Phylum_Abundance.csv")
cPhylum <- mutate(Phylum,
                 Color = c("#46095D","#440154","#26818E","#471164","#48196B","#24AA83","#3F4889","#4CC26C","#414287",
                           "#38B977","#482778","#63CB5F","#34618D","#1FA088","#443C84","#453581","#2D718E","#29AF7F",
                           "#21A585","#472E7C","#41BD72","#30B47C","#21908C","#7CD250","#6FCF57","#99D83E","#8AD647",
                           "#482071","#39558C","#375B8D","#E2E418","#A7DB35","#1E9B8A","#57C666","#3C4F8A","#B6DE2A",
                           "#297B8E","#C4E021","#25858E"), 
                 .after = "Total")
write.csv2(cPhylum, "/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_Phylum_Abundance_and_Color.csv")
```


```{r}
png("/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/Figura_10_Contribution_Distribution_Corrected_Phylum_Total_ASV.png", width = 17, height = 15, units = 'in', res = 600)
treemap(read.csv("/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_Phylum_Abundance_and_Color.csv", 
                 header = TRUE, 
                 sep = ";", 
                 dec = ".", 
                 skip = 0), 
        ...
        fontsize.labels = 20,
        fontsize.title = 30,
        title = "ASV contribution of rarefacted ASV at phylum level for TARA oceans",
        title.legend = NA,
        border.col = NA)
dev.off()

png("/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/Figura_10_Legend.png", width = 10, height = 7, units = 'in', res = 600)
plot.new()
par(mar = c(0,0,0,0), oma = c(0,0,0,0))
legend("center", 
       legend = read.csv("/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_Phylum_Abundance_and_Color.csv", 
                         header = TRUE, 
                         sep = ";", 
                         dec = ".", 
                         skip = 0)[,1], 
...
```{r}
rltv.abun <- Phylum[,-22] %>% t() %>% as.data.frame() %>% rltv.Otu.Table() %>% as.data.frame()
write.csv2(rltv.abun, "/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_Relative_Abundance.csv")
```
...
div <- ggplot(div.long, aes(x = Sample, y = value, fill = Phylum)) + 
  ...
  scale_fill_manual(values = cPhylum$Color) + 
  theme_ipsum(base_size = 22, axis_col = "#000000", axis_title_size = 40)
div.legend <- cowplot::get_legend(div)
div <- div + theme(legend.position = "none")
ggsave(plot = div, 
       filename = "Figura_11_TARA_Phylum_Relative_Abundance.png", 
       path = "/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/", 
       width = 35, 
       height = 21, 
       dpi = 600)
ggsave(ggpubr::as_ggplot(div.legend), 
       filename = "Figura_11_legend.png", 
       path = "/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/")
g <- ggplot_build(div)
list <- unique(g$data[[1]]["fill"])
list <- list$fill
```


```{r}
ppe <- cPhylum[c(4,7,12,13,23,26),]
write.csv2(PPE, "TARA_PPE_Abundance_and_Color.csv")

png("/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/Figura_12_Contribution_Distribution_Phylum_Total_ASV.png", width = 17, height = 15, units = 'in', res = 600)
treemap(read.csv("/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_PPE_Abundance_and_Color.csv", 
                 header = TRUE, 
                 sep = ";", 
                 dec = ".", 
                 skip = 0), 
        ...
        fontsize.labels = 20,
        fontsize.title = 30,
        title = "ASV contribution of rarefacted PPE ASV at phylum level for TARA oceans",
        title.legend = NA,
        border.col = NA
)
dev.off()
...
rppe <- ppe[,-c(22,23)] %>% t() %>% as.data.frame()
rppe <- rltv.Otu.Table(rppe) %>% as.data.frame()
write.csv2(rltv.ppe, "TARA_PPE_Relative_Abundance.csv")
```
...
rppel <- reshape2::melt(rppe, id.vars = "Sample", variable.name = "Phylum")
div.3 <- ggplot(rppel, aes(x = Sample, y = value, fill = Phylum)) + 
  geom_bar(position = "fill", stat = "identity") + 
  labs(x = "Sites", y = "Relative Abundance", title = "Relative PPE ASV abundance at phylum level for rarefacted TARA oceans dataset") + 
  scale_fill_manual(values = ppe$Color) + 
  theme_ipsum(base_size = 22, axis_col = "#000000", axis_title_size = 40, plot_title_size = 40)
div.legend <- cowplot::get_legend(div.3)
div.3 <- div.3 + theme(legend.position = "none")
ggsave(plot = div.3, 
       filename = "Figura_13_TARA_Phylum_Relative_Abundance.png", 
       path = "/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/", 
       width = 35, 
       height = 21, 
       dpi = 600)
