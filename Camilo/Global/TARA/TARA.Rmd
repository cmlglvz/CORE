...
Phylum <- Tax.sum(sTARA, sTAXA, 5) %>% as.data.frame()
Phylum <- Phylum[,-11]
...
phyl <- Phylum[,-22] %>% t() %>% as.data.frame()
rltv.abun <- rltv.Otu.Table(phyl) %>% as.data.frame()
...
```{r}
rltv <- rltv.abun
rltv <- add_column(rltv, Sample = rownames(rltv), .before = "Metazoa", .name_repair = "minimal")
...
```{r}
PhyCol <- mutate(Phylum,
                 Color = c("#440154", "#460B5E", "#481568", "#481D6F", "#482677", "#472F7D", "#453781", "#423F85", "#3F4788", "#3D4E8A", 
                           "#39558C", "#56C667", "#32648E", "#306A8E", "#2D718E", "#2A778E", "#287D8E", "#25838E", "#238A8D", "#21908C", 
                           "#1F968B", "#1E9D89", "#20A386", "#23A983", "#29AF7F", "#B8DE29", "#3CBC75", "#48C16E", "#365D8D", "#64CB5F", 
                           "#74D055", "#84D44B", "#94D840", "#A6DB35", "#32B67A", "#CAE11F", "#DCE318", "#EDE51B", "#FDE725"), 
                 .after = "Total")
write.csv2(PhyCol, "/Documents/GitHub/CORE/Camilo/Global/TARA/Phylum_TARA_Abundance_and_Color.csv")

cPhylum <- mutate(Phylum,
                 Color = c("#440154","#46095D","#26818E","#471164","#48196B","#24AA83","#3F4889","#4CC26C","#414287","#38B977",
                           "#482778","#63CB5F","#34618D","#1FA088","#453581","#443C84","#29AF7F","#2D718E","#21A585","#472E7C",
                           "#41BD72","#30B47C","#21908C","#7CD250","#6FCF57","#99D83E","#8AD647","#482071","#375B8D","#39558C",
                           "#E2E418","#A7DB35","#57C666","#1E9B8A","#3C4F8A","#297B8E","#B6DE2A","#C4E021","#25858E"), 
                 .after = "Total")
write.csv2(cPhylum, "/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_Phylum_Abundance_and_Color.csv")

png("/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/Fig_5_Contribution_Distribution_Corrected_Phylum_Total_ASV.png", width = 17, height = 15, units = 'in', res = 600)
treemap(read.csv("/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_Phylum_Abundance_and_Color.csv", 
        ...
        fontsize.labels = 20,
        fontsize.title = 30,
        title = "Contribution of total ASV",
        title.legend = NA,
        border.col = NA
)
dev.off()

png("/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/Fig_2_Legend.png", width = 10, height = 7, units = 'in', res = 600)
plot.new()
...
```{r}
div.2 <- ggplot(div.long, aes(x = Sample, y = value, fill = Phylum)) + 
  geom_bar(position = "fill", stat = "identity") + 
  scale_fill_manual(values = PhyCol$Color) + 
  theme_ipsum()
div.legend <- cowplot::get_legend(div.2)
div.2 <- div.2 + theme(legend.position = "none")
ggsave(plot = div.2, 
       filename = "Fig_3_TARA_Phylum_Relative_Abundance.png", 
       path = "/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/", 
       width = 35, 
       height = 21, 
       dpi = 300)
ggsave(ggpubr::as_ggplot(div.legend), 
       filename = "Fig_3_legend.png", 
       path = "/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/")
```
...
write.csv2(PPE, "/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_PPE_Abundance_and_Color.csv")

png("/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/Fig_6_Contribution_Distribution_Phylum_Total_ASV.png", width = 17, height = 15, units = 'in', res = 600)
treemap(read.csv("/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_PPE_Abundance_and_Color.csv", 
        ...
        fontsize.labels = 20,
        fontsize.title = 30,
        title = "Contribution of PPE ASV",
        title.legend = NA,
        border.col = NA
)
dev.off()

ppe <- PPE[,-c(22,23)] %>% t() %>% as.data.frame()
rltv.ppe <- rltv.Otu.Table(ppe) %>% as.data.frame()
write.csv2(rltv.ppe, "/Documents/GitHub/CORE/Camilo/Global/TARA/TARA_PPE_Relative_Abundance.csv")
```


```{r}
pperel <- add_column(rltv.ppe, Sample = rownames(rltv.ppe), .before = "Ochrophyta", .name_repair = "minimal")
pperel$Sample <- factor(pperel$Sample, levels = pperel$Sample)
pperel.l <- reshape2::melt(pperel, id.vars = "Sample", variable.name = "Phylum")
div.3 <- ggplot(pperel.l, aes(x = Sample, y = value, fill = Phylum)) + 
  geom_bar(position = "fill", stat = "identity") + 
...
ggsave(plot = div.3, 
       filename = "Fig_7_TARA_Phylum_Relative_Abundance.png", 
       path = "/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/", 
       width = 35, 
       height = 21, 
       dpi = 300)
ggsave(ggpubr::as_ggplot(div.legend), 
       filename = "Fig_7_legend.png", 
       path = "/Documents/GitHub/CORE/Camilo/Global/TARA/Figuras/")
```
