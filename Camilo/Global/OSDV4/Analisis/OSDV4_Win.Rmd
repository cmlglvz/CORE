...
```{r}
PreU <- as.data.frame(t(OSDV4))
PreU <- mutate(PreU, Seq = all_of(rownames(PreU)), .before = "OSD143")
PreU <- mutate(PreU, Total = rowSums(PreU[2:22]))
pre.ats <- inner_join(PreU, TAXA, by = "Seq") %>% relocate(ASV, .after = Seq)
rownames(pre.ats) <- all_of(pre.ats[,1])
write.csv2(pre.ats, "original_unfiltered_taxonomic_and_abundance_OSDV4.csv")
