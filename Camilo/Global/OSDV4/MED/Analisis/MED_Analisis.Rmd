...
MED <- read.csv("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/DADA2/seqtab_nonchim_med.csv", 
                header = TRUE, 
                sep = ";", 
                skip = 0, 
                fill = TRUE, 
                row.names = 1)
MED <- apply(MED, 2, function(x) as.numeric(as.integer(x))) %>% as.data.frame()
samples <- c("OSD014", "OSD022", "OSD099", "OSD049", "OSD076", "OSD077", "OSD123", "OSD132")
rownames(MED) <- samples
write.csv2(MED, "MED_ASV_Abundance.csv")

TAXA <- read.csv("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/DADA2/taxa_osd_v4_med.csv", 
                 header = TRUE, 
                 sep = ";", 
                 skip = 0) %>% 
  dplyr::rename(Seq = X) %>%
  add_column(ASV = c(str_glue("ASV000{1:9}"), 
                     ...
                     str_glue("ASV{1000:1015}")
                     ), 
             .after = "Seq")
rownames(TAXA) <- TAXA[,1]
write.csv2(TAXA, "OSD_V4_MED_TAXA.csv")

print(all(colnames(MED)%in%rownames(TAXA))) #If TRUE you can continue
```
...
write.csv2(mTAXA, "Filtered_MED_TAXA.csv")
mMED <- dplyr::select(MED, -all_of(na.seq))
write.csv2(mMED, "Filtered_MED_ASV_Abundance.csv")

print(all(colnames(mMED)%in%rownames(mTAXA))) #If TRUE you can continue
```


#Rarefacción
```{r Rarefaction, echo=TRUE, message=TRUE, warning=TRUE}
rare.all <- sort(rowSums(mMED)) #To identify the sample with the smallest number of observations, which will be used as the number of observations to rarefy samples

rrfy <- rarefy(mMED, rare.all[1]) #rarefaction use the smallest number of observations per sample to extrapolate the expected number if all other samples only had that number of observations
...
png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Imagenes/Fig_1_Rarecurve_mMED.png", width = 12, height = 6, units = "in", res = 300)
par(mar = c(5.1, 4.1, 4.1, 2.1), oma = c(0, 0, 0, 0))
rarecurve(x = mMED, col = "green", lwd = 2, las = 1, label = TRUE, step = 10, cex = 0.5)
dev.off()

rareMED <- as.data.frame(rrarefy(mMED, rrfy[1]))
write.csv2(rareMED, file = "Rarefacted_filt_MED_ASV_Abundance.csv")
```


#Unify abundance and taxonomic dataframes
```{r}
Union <- as.data.frame(t(mMED))
Union <- mutate(Union, Seq = all_of(rownames(Union)), .before = "OSD014")
Union <- mutate(Union, 
                Total = rowSums(Union[2:9])
                )
ATs <- inner_join(Union, mTAXA, by = "Seq") %>% relocate(ASV, .after = Seq)
rownames(ATs) <- all_of(ATs[,1])
write.csv2(ATs, "Taxonomic_and_Abundance_MED.csv")
```
...
asv <- ATs[,2]
edited <- ATs[, -c(2,11:19)]
rownames(edited) <- asv
write.csv2(edited, "edited_Taxonomic_and_Abundance_MED.csv")
APATs <- edited[, -1]
APATs <- vegan::decostand(APATs, method = "pa")
APATs <- add_column(APATs, Seq = all_of(edited$Seq), .before = "OSD014")
write.csv2(APATs, "Absence_Presence_MED.csv")
```


#Complete UpSet
```{r Complete, echo=TRUE, message=TRUE, warning=TRUE}
aus.pres <- read.csv("Absence_Presence_MED.csv", 
                     header = TRUE, 
                     sep = ";", 
                     dec = ".", 
                     skip = 0)

png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Imagenes/Fig_2_Complete_UpSet.png", width = 40, height = 17, units = "in", res = 200)
upset(aus.pres, 
      nsets = 8, 
      nintersects = NA, 
...
png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Imagenes/Fig_3_Shorter_UpSet.png", width = 30, height = 13, units = "in", res = 200)
upset(aus.pres, 
      nsets = 8,
      nintersects = NA, 
      sets = c("OSD132", "OSD123", "OSD077", "OSD076", "OSD049", "OSD099", "OSD022", "OSD014"), 
      intersections = list("OSD132", "OSD123", "OSD077", "OSD076", "OSD049", "OSD099", "OSD022", "OSD014", 
                           ...
                           list("OSD049", "OSD099", "OSD022"), 
                           list("OSD049", "OSD099", "OSD022", "OSD014"), 
...
Phylum <- Tax.sum(mMED, mTAXA, 5) %>% 
  t() %>% 
  as.data.frame() %>% 
  dplyr::mutate(Color = c("#FDFBE3", "#FDFBE3", "#000000", "#24F205", "#FDFBE3", "#60F649", "#88F877", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                          "#C8FBC1", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#AFFAA4", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                          "#88F877", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#E0FDDC", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                          "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3")
                )
Phylum <- dplyr::mutate(Phylum, 
                        Total = rowSums(Phylum[1:8]), 
                        .after = "OSD132")
Phylum <- Phylum[order(Phylum$Total, decreasing = TRUE),]
write.csv2(Phylum, "Phylum_MED_Abundance_and_color.csv")
```


```{r phylum visualization, echo=TRUE,message=TRUE,warning=TRUE}
png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Imagenes/Fig_4_Contribution_Distribution_Phylum.png", width = 17, height = 15, units = "in", res = 300)
treemap::treemap(read.csv("Phylum_MED_Abundance_and_color.csv", 
                          header = TRUE, 
                          sep = ";", 
                          dec = ".", 
                          skip = 0), 
...
png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Imagenes/Fig_4_Legend.png", width = 10, height = 7, units = "in", res = 300)
plot.new()
par(mar = c(0,0,0,0), oma = c(0,0,0,0))
legend("center", 
       legend = read.csv("Phylum_MED_Abundance_and_color.csv", 
                         header = TRUE, 
                         sep = ";", 
                         dec = ".", 
                         skip = 0)[,1], 
...
```{r}
rltv.abun <- rltv.Otu.Table(Tax.sum(mMED, mTAXA, 5)) %>% as.data.frame()
write.csv2(rltv.abun, "MED_Relative_Abundance_dataframe.csv")
```


```{r}
png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Imagenes/Fig_5_MED_Relative_Abundance.png", width = 30, height = 11, units = "in", res = 300)
par(mar = c(5.1,4.1,4.1,2.1), oma = c(0,0,0,0))
barplot(t(rltv.abun), 
        border = NA, 
        ...
        ylim = c(0,1), 
        axes = TRUE, 
        col = c("#FDFBE3", "#000000", "#FDFBE3", "#24F205", "#FDFBE3", "#60F649", "#88F877", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                "#C8FBC1", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#AFFAA4", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                "#88F877", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#E0FDDC", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3"), 
        las = 2, 
...
png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Imagenes/Fig_5_Legend.png", width = 3, height = 5, units = "in", res = 300)
plot.new()
par(mar = c(0,0,0,0), oma = c(0,0,0,0))
legend("center", 
       ...
       cex = 1, 
       ncol = 1, 
       fill = c("#FDFBE3", "#000000", "#FDFBE3", "#24F205", "#FDFBE3", "#60F649", "#88F877", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                "#C8FBC1", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#AFFAA4", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                "#88F877", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#E0FDDC", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3"), 
       x.intersp = 0.1, 
...
write.csv2(ppe.taxa, "/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Analisis/PPE/MED_PPE_TAXA.csv")
ppe.med <- select(mMED, all_of(ppe.taxa$Seq))
write.csv2(ppe.med, "/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Analisis/PPE/MED_PPE_ASV_Abundance.csv")
ppe.un <- as.data.frame(t(ppe.med)) %>% dplyr::mutate(Seq = all_of(ppe.taxa$Seq), .before = "OSD014")
ppe.un <- mutate(ppe.un, 
                 Total = rowSums(ppe.un[2:9])
                 )
ppe.at <- inner_join(ppe.un, ppe.taxa, by = "Seq") %>% relocate(ASV, .after = Seq)
rownames(ppe.at) <- all_of(ppe.at[,1])
write.csv2(ppe.at, "/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Analisis/PPE/Taxonomic_and_Abundance_PPE.csv")
```


```{r}
verdes <- ppe.at[,2]
vedited <- ppe.at[, -c(2,11:19)]
rownames(vedited) <- verdes
write.csv2(vedited, "/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Analisis/PPE/edited_taxonomic_and_abundance_PPE.csv")
ppe.apats <- vedited[,-1]
ppe.apats <- vegan::decostand(ppe.apats, method = "pa") %>% add_column(Seq = all_of(vedited$Seq), .before = "OSD014")
write.csv2(ppe.apats, "/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Analisis/PPE/Absence_Presence_PPE.csv")
```


#PPE UpSet
```{r}
aps.ppe <- read.csv2("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Analisis/PPE/Absence_Presence_PPE.csv", 
                     header = TRUE, 
                     sep = ";", 
                     dec = ".", 
                     skip = 0)

png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Global/OSDV4/MED/Imagenes/PPE/Fig_6_Complete_PPE_UpSet.png", width = 40, height = 17, units = "in", res = 200)
upset(aps.ppe, 
      nsets = 8, 
      nintersects = NA, 
      sets = c("OSD132", "OSD123", "OSD077", "OSD076", "OSD049", "OSD099", "OSD022", "OSD014"), 
