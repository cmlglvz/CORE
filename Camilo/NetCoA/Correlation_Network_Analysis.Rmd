...
setwd("~/Documents/GitHub/CORE/Camilo/NetCoA/")
node_t <- list()
for (y in 1:6) {
  ...
  write.table(tcor$r, str_glue("~/Documents/GitHub/CORE/Camilo/NetCoA/net_creation/t{y}/tr{y}.csv"), sep = "\t", dec = ".")
  #Guardamos el resultado con valores de p (una tabla)
  write.table(tcor$p, str_glue("~/Documents/GitHub/CORE/Camilo/NetCoA/net_creation/t{y}/tr{y}.csv"), sep = "\t", dec = ".")
  ...
  write.table(node_t[[y]],str_glue("~/Documents/GitHub/CORE/Camilo/NetCoA/net_creation/t{y}/node_mat_t{y}.csv",sep = "\t",dec = ".",row.names = FALSE))
  write.table(tcytos,str_glue("~/Documents/GitHub/CORE/Camilo/NetCoA/net_creation/t{y}/adj_mat_t{y}.csv"),sep = "\t",dec = ".",row.names = FALSE)
}
...
for (i in 1:dim(path_u)[1]) {
  ...
  write.table(node_t_class[[i]],str_glue("~/Documents/GitHub/CORE/Camilo/NetCoA/net_creation/t{i}/node_mat_t{i}.csv"), sep = "\t", dec = ".")
}
...
write.table(ASVs2, "~/Documents/GitHub/CORE/Camilo/NetCoA/net_creation/other_results/ASVs_class.csv", row.names = FALSE)
```
...
graphlist <- list()
for (x in 1:6) {
  nodelist[[x]] <- read.table(str_glue("~/Documents/GitHub/CORE/Camilo/NetCoA/net_creation/t{x}/node_mat_t{x}.csv"), sep = "\t", dec = ".", header = TRUE) %>% relocate(Node)
  data <- read.table(str_glue("~/Documents/GitHub/CORE/Camilo/NetCoA/net_creation/t{x}/adj_mat_t{x}.csv"), sep = "\t", dec = ".", header = TRUE)
...
degs <- degs %>% bind_rows()
deg2 <- deg %>% bind_rows()
ggplot(degs, aes(y = Total, x = Net, fill = factor(Var1, levels = c("Other", "Unique", "Shared")))) + 
  geom_col() + 
  scale_fill_manual(name = "ASV Category", values = c("#2B2D42", "#DCE2E5", "#EF233C"))
