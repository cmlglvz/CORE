...
Phylum <- Tax.sum(mCORE, mTAXA, 5) %>% 
  t() %>% 
  as.data.frame() %>% 
  dplyr::mutate(Color = c("#440154", "#460C5F", "#481769", "#482071", "#482979", "#46337E", "#443B84", "#414487", "#3E4C8A", 
                          "#3A548C", "#365C8D", "#33638D", "#306A8E", "#2C718E", "#2A788E", "#277F8E", "#24868E", "#228D8D", 
                          "#20938C", "#1F9A8A", "#1FA187", "#22A884", "#28AE80", "#32B67A", "#3DBC74", "#4AC16D", "#59C864", 
                          "#68CD5B", "#7AD151", "#8CD646", "#9FDA3A", "#B2DD2D", "#C5E021", "#D9E219", "#EBE51B", "#FDE725")
                )
...
```{r}
filum <- Tax.sum(mCORE, mTAXA, 5) %>% as.data.frame()
filum <- filum[,-22]
colnames(filum)[c(6,13)] <- c("A","B")
filum <- mutate(filum, 
                Cryptophyta = rowSums(filum[c(6,13)]))
filum <- filum[,-c(6,13)]
filum <- as.data.frame(t(filum))
filum <- mutate(filum, 
                Chanaral = rowSums(filum[c(1:12)]), 
                Flamenco = rowSums(filum[c(13:24)]),
                Huasco = rowSums(filum[c(25:36)]),
                Pta.Choros = rowSums(filum[c(37:41)]),
                Quintero = rowSums(filum[c(42:51)]),
                LasCruces = rowSums(filum[c(52:63)]),
                Total = rowSums(filum[c(1:63)]),
                .after = "L4A18")
filum <- filum[order(filum$Total, decreasing = TRUE),]
write.csv2(filum, "/Documents/GitHub/CORE/Camilo/Analisis/HCS_Corrected_Phylum_Abundance.csv")
```
...
```{r}
rltv.div <- rltv.abun
rltv.div <- add_column(rltv.div, Sample = rownames(rltv.div), .before = "Chlorophyta", .name_repair = "minimal")
...
ggsave(plot = div, 
       filename = "Fig_X_CORE_Phylum_Relative_Abundance.png", 
       path = "/Users/Artemis/Documents/GitHub/CORE/Camilo/Imagenes/", 
       width = 35, 
       height = 21, 
       dpi = 300)
ggsave(ggpubr::as_ggplot(div.legend), 
       filename = "Fig_X_legend.png", 
       path = "/Users/Artemis/Documents/GitHub/CORE/Camilo/Imagenes/")
g <- ggplot_build(div)
...
```{r}
png("/Users/Artemis/Documents/GitHub/CORE/Camilo/Imagenes/PPE/Fig_7_Selected_PPE_UpSet.png", width = 30, height = 13, units = "in", res = 300)
upset(aps.ppe,
      nsets = 6, 
      nintersects = NA, 
      sets = c("Chanaral", "Flamenco", "Huasco", "Pta.Choros", "Quintero", "LasCruces"), 
      intersections = list("LasCruces", "Quintero", "Pta.Choros", "Huasco", "Flamenco", "Chanaral", 
                           ...
                           list("Huasco", "LasCruces"),
                           list("Chanaral", "Huasco"), 
                           list("Chanaral", "Quintero"), 
                           list("Flamenco", "Quintero"), 
                           list("Pta.Choros", "LasCruces"), 
                           list("Chanaral", "Flamenco", "Huasco"), 
                           list("Huasco", "Pta.Choros", "LasCruces"), 
                           list("Chanaral", "Flamenco", "Huasco", "Pta.Choros"),
                           list("Chanaral", "Flamenco", "Huasco", "Pta.Choros", "Quintero"), 
                           list("Chanaral", "Flamenco", "Huasco", "Quintero", "LasCruces"), 
                           list("Chanaral", "Flamenco", "Huasco", "Pta.Choros", "Quintero", "LasCruces")
                           ),
      empty.intersections = "on", 
      keep.order = TRUE, 
      query.legend = "top", 
      queries = list(list(query = intersects, params = "Chanaral", color = "#D47B2E", active = TRUE, query.name = "Unique Chañaral ASV"), 
                     list(query = intersects, params = "Flamenco", color = "#F2C066", active = TRUE, query.name = "Unique Flamenco ASV"), 
                     list(query = intersects, params = "Huasco", color = "#16387B", active = TRUE, query.name = "Unique Huasco ASV"), 
                     list(query = intersects, params = "Pta.Choros", color = "#8BE2FC", active = TRUE, query.name = "Unique Punta Choros ASV"), 
                     list(query = intersects, params = "Quintero", color = "#EC4E44", active = TRUE, query.name = "Unique Quintero ASV"), 
                     list(query = intersects, params = "LasCruces", color = "#E96B67", active = TRUE, query.name = "Unique Las Cruces ASV"), 
                     list(query = intersects, params = list("Chanaral", "Flamenco", "Huasco", "Pta.Choros"), color = "#F1F500", active = TRUE, query.name = "Metal axis shared ASV"), 
                     list(query = intersects, params = list("Chanaral", "Flamenco", "Huasco", "Pta.Choros", "Quintero", "LasCruces"), color = "#007F5F", active = TRUE, query.name = "Total shared ASV")
                     ), 
      point.size = 3.5, 
      line.size = 1.5, 
      text.scale = 2, 
