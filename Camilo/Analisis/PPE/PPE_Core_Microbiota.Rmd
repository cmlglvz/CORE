...
#Getting rid off the "macro" ASV
ppe.taxa <- dplyr::filter(ppe.taxa, Family != "Phaeophyceae_XX") #1281 ASV
...
others <- dplyr::select(HCS, -all_of(shappe$Seq)) %>% 
  dplyr::select(-all_of(u.chappe$Seq)) %>% 
...
write.csv2(others, "HCS_Others_PPE_ASV_Abundance")
