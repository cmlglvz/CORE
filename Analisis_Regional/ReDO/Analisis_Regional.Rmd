...
CORE <- read.csv("/Users/Artemis/Documents/GitHub/CORE/DADA2_Regional/seqtab_nonchim.csv", 
                 header = TRUE, 
                 sep = ";", 
                 skip = 0, 
                 fill = TRUE, 
...
TAXA <- read.csv("/Users/Artemis/Documents/GitHub/CORE/DADA2_Regional/taxa.csv", 
                 header = TRUE, 
                 sep = ";", 
                 skip = 0) %>% 
  dplyr::rename(Seq = X) %>% 
...
naTAXA <- TAXA[!(row.names(TAXA) %in% all_of(NASeq)), ]
write.csv2(naTAXA, "selected_TAXA.csv")
naCORE <- select(CORE, -all_of(NASeq))
print(all(colnames(naCORE)%in%rownames(naTAXA)))
write.csv2(naCORE, "selected_CORE_ASV.csv")
```


##Rarefacción
```{r}
rare.full <- sort(rowSums(naCORE))
rare.full
plot(rare.full)

rrfy<- rarefy(naCORE, rare.full[1]) #rarefaction uses the smallest number of observations per sample to extrapolate the expected number if all other samples only had that number of observations
rrfy #gives an "expected"rarefied" number of species (not obs) if only 41641 individuals were present per sample

png("Rarecurve_CORE.png", width = 12, height = 6, units = 'in', res = 600)
par(mar=c(5.1,4.1,4.1,2.1), oma=c(0,0,0,0))
rarecurve(x = naCORE, col = "Grey", lwd = 2, las = 1, label = TRUE, step = 10, cex = 0.5)
dev.off()

rareCORE <- as.data.frame(rrarefy(naCORE, rrfy[1]))
write.csv2(rareCORE, file = "rarefacted_CORE_ASV.csv")
```


## Union de tablas
```{r union, echo=TRUE, message=TRUE, warning=TRUE}
Union <- as.data.frame(t(naCORE))
...
ATs <- inner_join(Union, naTAXA, by = "Seq") %>% relocate(ASV, .after = Seq)
...
write.csv2(APATs, "Ausencia_Presencia_selected_CORE.csv")
```


## UpSet para "todas" las ASV
```{r complete, echo=TRUE, message=TRUE, warning=TRUE}
aus.pres <- read.csv("Ausencia_Presencia_selected_CORE.csv", 
                     header = TRUE, 
                     sep = ";", 
                     dec = ".", 
                     skip = 0)
...
```{r}
unique(ATs$Division)
PPE.ATs <- dplyr::filter(ATs, Division == "Chlorophyta" | Division == "Ochrophyta" | Division == "Cryptophyta" | Division == "Cryptophyta:nucl" | Division == "Haptophyta" | Division == "Katablepharidophyta" | Division == "Rhodophyta")
write.csv2(PPE.ATs, "PPE_ATs_selected_CORE.csv")
ppe.otu <- PPE.ATs[, 2]
ppe.edited <- PPE.ATs[, -c(2, 72:80)]
rownames(ppe.edited) <- ppe.otu
write.csv2(ppe.edited, "edited_PPE_AT.csv")
PPE.APATs <- ppe.edited[, -1]
PPE.APATs <- decostand(PPE.APATs, method = "pa")
PPE.APATs <- add_column(PPE.APATs, Seq = all_of(ppe.edited$Seq), .before = "C1A17")
write.csv2(PPE.APATs, "Ausencia_Presencia_PPE_selected_CORE.csv")

ppe.aus.pres <- read.csv("Ausencia_Presencia_PPE_selected_CORE.csv", 
                     header = TRUE, 
                     sep = ";", 
                     dec = ".", 
                     skip = 0)

png("Complete_UpSet_PPE_ASV.png", width = 50, height = 18, units = "in", res = 600)
upset(ppe.aus.pres, 
      nsets = 9, 
      nintersects = NA, 
      sets = c("Cha", "Fla", "Hu", "Pc", "LC", "Qin"), 
...
png("Selected_UpSet_PPE_ASV.png", width = 30, height = 15, units = "in", res = 600)
upset(ppe.aus.pres, 
      nsets = 6, 
      nintersects = NA, 
      sets = c("Cha", "Fla", "Hu", "Pc", "LC", "Qin"), 
...
Phylum <- Tax.sum(naCORE, naTAXA, 5) %>% 
  t() %>% 
  as.data.frame() %>% 
  mutate(Color = c("#24F205", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#60F649", "#88F877", "#FDFBE3", "#AFFAA4", "#FDFBE3", "#FDFBE3", 
                   "#FDFBE3", "#FDFBE3", "#88F877", "#C8FBC1", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
                   "#FDFBE3", "#000000", "#E0FDDC", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", "#FDFBE3", 
...
png("Contribution_Distribution_Phylum_Total_ASV.png", width = 17, height = 15, units = 'in', res = 300)
...
png("Contribution_Distribution_Phylum_Total_ASV_Legend.png", width = 10, height = 7, units = 'in', res = 600)
plot.new()
...
#UpSet para PPE
ppe.taxa <- filter(naTAXA, Division == "Chlorophyta" | Division == "Ochrophyta" | Division == "Cryptophyta" | Division == "Cryptophyta:nucl" | Division == "Haptophyta" | Division == "Katablepharidophyta" | Division == "Rhodophyta")
ppe.asv <- select(naCORE, all_of(ppe.taxa$Seq))
ppe.aus.pres <- filter(aus.pres, Seq %in% all_of(ppe.taxa$Seq))
