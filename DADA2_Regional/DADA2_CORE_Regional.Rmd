...
path <- "X:/Documents/CORE/Secuencias/"
fns <- list.files(path)
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, 
                     truncLen=c(240,210), trimLeft=c(15,15), 
...
class(mergers) #list
length(mergers) #72 elements in the list, one for each sample
names(mergers)
#head(mergers[[1]])
```
...
taxa <- assignTaxonomy(seqtab.nochim, "F:/R/DBs/pr2_version_4.14.0_SSU_dada2.fasta.gz", taxLevels = c("Kingdom", "Supergroup", "Division", "Class", "Order", "Family", "Genus", "Species"), multithread = TRUE)
```
...
```{r DECIPHER tax, message=TRUE, warning=TRUE}
trained <- readRDS("E:/R/DBs/pr2_version_4.14.0_SSU.decipher.trained.rds") #Already trained object for DECIPHER to assign taxa
seqs <- DNAStringSet(getSequences(seqtab.nochim)) #DNAstring object for ASVs (efficient storage and manipulation of long sequences)
tax_info <- IdTaxa(test = seqs,
                   trainingSet = trained, 
                   strand = "both", 
                   threshold = 70,
                   processors = NULL)

#Time difference of 1093.66 secs
```
...
assignment <- sapply(tax_info, function(x) paste(x$taxon, collapse=";"))
...
levels <- c("Kingdom", "Supergroup", "Division", "Class", "Order", "Family", "Genus", "Species")
